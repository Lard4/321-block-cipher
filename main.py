...
def pkcs7(block, blocksize):
    # determine number of bytes to pad
    padBytes = blocksize - len(block)
    if padBytes == 0:
        padBytes = blocksize

    # pad the byte representation of the number of pad bytes until block filled
...
def testTaskI():
    ...
    print(" CBC key:", global_key.hex())
    print(" CBC IV:", global_iv.hex())
    ...
    print(yellow, "diff CBC_correct.bmp CBC_custom.bmp", end="")
...
def testTaskII():
    ...
    userString = input(" Please provide a string to encrypt: ")
    cipher = submit("You're the man now; =, ; = dog")

    result = verify(cipher)
    print(" verify() returned ", end="")
    if not result:
        print(green, end="")
    else:
        print(red, end="")
